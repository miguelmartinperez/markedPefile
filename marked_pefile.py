import re
from .pefile.pefile import PE, two_way_dict, MAX_SYMBOL_EXPORT_COUNT, OPTIONAL_HEADER_MAGIC_PE, \
    OPTIONAL_HEADER_MAGIC_PE_PLUS, Structure, SectionStructure, UNW_FLAG_CHAININFO, PEFormatError
import pdb
...
class MarkedPE(PE):
    def __init__(self, name=None, data=None, fast_load=None, max_symbol_exports=MAX_SYMBOL_EXPORT_COUNT,
                 virtual_layout=False, valid_pages=None, base_address=None, architecture=None):
        try:
            super(MarkedPE, self).__init__(name=name, data=data, fast_load=fast_load,
                                           max_symbol_exports=max_symbol_exports, virtual_layout=virtual_layout)
    ...
    def set_visited(self, object=None, tag=None, force=False, pointer=None, size=None):
        ...
        while index < pointer + size:
            ...
            if self.__visited__[index] == MARKS['UNKW_BYTE'] or self.__visited__[index] == tag or force:
                ...
                logging.warning('Visiting space (Module address:{} Offset:{}) as {} previously visited as {}'.format(
                    self.__base_address__, index, MARKS[tag], MARKS[self.__visited__[index]]))
            index += 1
    ...
    def visit_unwind(self, UnwindInfoStruct):
        ...
        self.set_visited(pointer=UnwindInfoStruct.get_file_offset(),
                         size=unwind_init_size + ((UnwindInfoStruct.CountOfUnwindCode + 1) & ~1) * code_size,
                         tag=MARKS['IMAGE_DIRECTORY_ENTRY_EXCEPTION'])

        if UnwindInfoStruct.Flags & UNW_FLAG_CHAININFO:
            self.set_visited(pointer=UnwindInfoStruct.get_file_offset() + unwind_init_size + (
                    (UnwindInfoStruct.CountOfUnwindCode + 1) & ~1) * code_size, size=untime_function_size)
    ...
    def set_zero_word(self, address):
        # self.__data__ = self.__data__[:address + 2] + '\x00\x00' + self.__data__[address + 4:] # Python 2
        self.__data__ = self.__data__[:address + 2] + b'\x00\x00' + self.__data__[address + 4:]

    def set_zero_double_word(self, address):
        # self.__data__ = self.__data__[:address + 2] + '\x00\x00\x00\x00\x00\x00' + self.__data__[address + 8:] # Python 2
        self.__data__ = self.__data__[:address + 2] + b'\x00\x00\x00\x00\x00\x00' + self.__data__[address + 8:]

    def get_section_by_name(self, section_name):
        for section in self.sections:
            # if re.match(section_name, section.Name): Python 2
            if re.match(bytes(section_name, 'utf-8'), section.Name):
    ...
    def marking(self):
        ...
        # null_address = '\x00\x00\x00\x00' if self.PE_TYPE == OPTIONAL_HEADER_MAGIC_PE else '\x00\x00\x00\x00\x00\x00\x00\x00' # Python 2
        null_address = b'\x00\x00\x00\x00' if self.PE_TYPE == OPTIONAL_HEADER_MAGIC_PE else b'\x00\x00\x00\x00\x00\x00\x00\x00'
        ...
        for directory in self.OPTIONAL_HEADER.DATA_DIRECTORY:
            ...
            if directory.VirtualAddress and directory.Size:
                if directory.name == 'IMAGE_DIRECTORY_ENTRY_SECURITY':
                    ...
                    self.set_visited(pointer=directory.VirtualAddress,
                                     size=self.DIRECTORY_ENTRY_LOAD_CONFIG.struct.Size,
        ...
        if hasattr(self, 'DIRECTORY_ENTRY_TLS'):
            if self.DIRECTORY_ENTRY_TLS.struct.AddressOfCallBacks:
                ...
                while True:
                    self.set_visited(pointer=AddressOfCallBacksIndex, size=address_size,
                                     tag=MARKS['IMAGE_DIRECTORY_ENTRY_TLS'])
        ...
        for section in self.sections:
            ...
            # print(f'Index left: {section.VirtualAddress + min(section.SizeOfRawData,section.Misc_VirtualSize)}')
            # print(f'Index right: {section.VirtualAddress + section.real_size}')
            # Transform indexes to int() in Python 3
            if all_zero(self.__data__[
                        int(section.VirtualAddress + min(section.SizeOfRawData, section.Misc_VirtualSize)):int(
                            section.VirtualAddress + section.real_size)]):
            ...
            elif all_zero(self.__data__[
                          int(section.VirtualAddress + max(section.SizeOfRawData, section.Misc_VirtualSize)):int(
                              section.VirtualAddress + section.real_size)]):
